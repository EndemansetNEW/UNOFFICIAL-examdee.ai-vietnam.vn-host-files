...
import { getCapabilities } from "chatkit-web-shared/capabilities"
import type { Capabilities, Capability } from "chatkit-web-shared/types/capabilities.types"
import { IntegrationError, fromPossibleFrameSafeError } from "chatkit-web-shared/errors"

const cs = document.currentScript as HTMLScriptElement | null
const scriptUrl = cs?.src || null
// Strip out the filename from the script url to get the chatkit url
const chatkitUrl = scriptUrl && scriptUrl.replace(/\/[^/]*$/, "/").toString()
const CHATKIT_INNER_ENTRYPOINT = import.meta.env.CHATKIT_INNER_ENTRYPOINT

if (!chatkitUrl || !CHATKIT_INNER_ENTRYPOINT) {
  throw new Error(
    "Unable to determine the ChatKit frame URL. Please ensure that the ChatKit script is loaded correctly.",
  )
}

const CHATKIT_FRAME_URL = new URL(CHATKIT_INNER_ENTRYPOINT, chatkitUrl)
...
export abstract class ChatKitElementBase<TRawOptions> extends HTMLElement {
  ...
  #applyFrameTitle(options?: ChatKitOptions) {
    if (!this.#frame) return
    this.#frame.title = options?.frameTitle ?? "Chat"
  }
  ...
  connectedCallback() {
    ...
    this.#frame = frame
    this.#applyFrameTitle(this.#opts)
  ...
  #maybeInit() {
    ...
    if (this.#opts.externalVendors?.logRocket) {
      frameURL.searchParams.set("externalVendors", "logrocket")
    }
    frameURL.hash = encodeBase64({
      options: getInnerOptions(this.#opts),
      referrer: window.location.origin,
      profile: this.profile,
      iframeInitStartedAt: Date.now(),
  ...
  protected applySanitizedOptions(newOptions: ChatKitOptions) {
    ...
    this.#applyFrameTitle(newOptions)
    if (this.#initialized) {
      this.#setOptionsDataAttributes(this.#opts)
      this.#loaded.then(() => {
